<template>
    <el-tabs type="border-card" class="demo-tabs" v-model="curTab" @tab-click="handleTab">
            ...
            <el-form-item>
                ...
                <el-form-item :label="$t('时间') + '：'" style="margin-left:-40px;">
                    <el-date-picker v-model="form.time" type="daterange" unlink-panels range-separator="To"
                        start-placeholder="Start date" end-placeholder="End date" style="margin-left:-10px;"
        ...
        <el-tab-pane :label="$t('历史数据分布')">
            <EChartsWrapper :title="$t('历史数据分布统计')" :xAxis-data="labels" :series-data="chartData" />
        </el-tab-pane>
    </el-tabs>
</template>
<script setup>
...
onMounted(async () => {
    try {
        ...
        await getInfo("centre/getHistoryOnlineInfo").then((res) => {
...
const search = async () => {
    ...
    await getInfo("centre/getHistoryOnlineInfo", options).then((res) => {
        labels.value = res.date || [];
        chartData.value = [];
        chartData.value.push({
            name: `最大值数据`,
