<template>
  ...
  <el-drawer
    v-model="table"
    :title="chooseDate+$t('详细信息')"
    direction="rtl"
    size="60%"
  >
    <DynamicTable :columns="drawerColumns" :data="drawerTableData"  :delete-btn="false"
      :operational="false" :selection="false" :pagination="false" />
  </el-drawer>
</template>
...
const loadData = async (page) => {
  ...
  const getData = await getInfo("centre/getCreationList", { Page: page,...options });
...
const columns = [
  { label:'日期', prop: 'Date', linkable: true, linkStyle: 'custom-link' },
  { label:'注册数量', prop: 'RegNums' },
  { label:'滚服注册数量', prop: 'GunRegNums' },
  { label:'总角色数', prop: 'RoleNums' },
  { label:'滚服角色数', prop: 'GunRoleNums' },
  { label:'滚服率', prop:'GunRate' },
  { label:'总创建率', prop:'CreationRate' },
  { label:'新用户创建率',prop:'NewCreationRate' },
];

const drawerColumns = [
  { 
    label:'时间[小时]', prop: 'Time', formatter: (row, column, cellValue) => { 
      return Number(row.Time) + '-' + (Number(row.Time)+1);
    },
  },
  { label:'注册数量', prop: 'RegNums' },
  { label:'创建数量', prop: 'RoleNums' },
  { label:'创建率', prop: 'CreationRate' },
];

onMounted(async () => {
  try {
    isLoading.value = true;
    await getInfo("centre/getCreationList").then((res) => {
      isLoading.value = false;
...
const exportInfo = async () => {
  ...
  await exportGetCSV('centre/getCreationList',{
    Limit: total.value,
    Page: 1,
    ...options
  }, columns, writer, `${t("角色创建统计")}_${form.platID}_${form.serverID}_${Date.now()}.csv`).catch(async error => {
...
const handleTableLinkClick = async({ row, prop }) => {
  chooseDate.value = row.Date;
  await getInfo("centre/getCreationDetail",{
    ServerID: form.serverID,
    GameOperator: form.platID,
    DetailDate: row.Date,
  }).then(res => {
