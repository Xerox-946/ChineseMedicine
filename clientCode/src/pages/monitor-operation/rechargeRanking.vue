...
const loadData = async (page) => {
  ...
  const getData = await getInfo("centre/getOrderRank", { Page: page,...options });
...
const columns = [
  ...
  { 
      ...
      const server = serverData.value.find(s => s.server_id === row.ServerID);
      return server ? server.server_name : '未知服务器';
    } 
  },
  { 
      label:'付费平台', prop: 'GameOperator',formatter: (row, column, cellValue) => {
      const plat = platData.value.find(p => p.platform_id == Number(row.GameOperator));
      return plat ? t(plat.name) : '未知平台';
    } 
  },
]
...
onMounted(async () => {
  try {
    isLoading.value = true;
    await getInfo("centre/getOrderRank").then((res) => {
      isLoading.value = false;
...
const exportInfo = async () => {
  ...
  await exportGetCSV('centre/getOrderRank',{
    Limit: total.value,
    Page: 1,
    ...options
  }, columns, writer, `${t("付费排行")}_${form.platID}_${form.serverID}_${Date.now()}.csv`).catch(async error => {
