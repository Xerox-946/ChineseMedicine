  ...
  <el-tabs type="border-card" class="demo-tabs" v-model="curTab" @tab-click="handleTab">
    <el-form :model="form" label-width="auto">
      <el-form-item>
        ...
        <el-form-item :label="$t('领主ID') + '：'">
          <el-input v-model="form.rno" style="width:100px;" clearable />
        </el-form-item>
        <el-form-item :label="$t('角色名') + '：'" >
          <el-input v-model="form.roleName" style="width:100px;" clearable />
        </el-form-item>
        <el-form-item :label="$t('状态') + '：'">
          <el-select v-model="form.isSuccess" style="width:100px;">
            <el-option :label="$t('全部')" value="0" />
...
const loadData = async (page) => {
  const options = {
    ...
    UserID: form.rno,
    UserName: form.roleName,
    Status:form.isSuccess!="0" && form.isSuccess ? 1:0,
    Page: page,
  }
...
const columns = [
  ...
  { 
    label:'付费状态', prop:'IsSuccess', formatter: (row, column, cellValue) => {
...
const exportInfo = async () => {
  ...
  await exportGetCSV('centre/getOrderInfoList',{
    ...
    Page: 1,
    ...options
  }, columns, writer, `${t('指定玩家付费')}_${Date.now()}.csv`).catch(async error => {
