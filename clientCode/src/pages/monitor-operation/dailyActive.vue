<template>
    <el-tabs type="border-card" class="demo-tabs" v-model="curTab" @tab-click="handleTab">
        ...
        <el-tab-pane :label="$t('每日活跃')">
            <EChartsWrapper :title="$t('每日活跃统计')" :xAxis-data="labels" :series-data="chartData" />
        </el-tab-pane>
    </el-tabs>
</template>
<script setup>
...
onMounted(async () => {
    try {
        ...
        await getInfo("centre/getDailyActiveInfo").then((res) => {
...
const search = async () => {
    ...
    await getInfo("centre/getDailyActiveInfo", options).then((res) => {
        labels.value = res.date || [];
        chartData.value = [];
        chartData.value.push({
            name: `数据`,
