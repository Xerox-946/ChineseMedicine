...
const loadData = async (page) => {
  ...
  const getData = await getInfo("centre/getOperationList", { Page: page, ...options });
...
const columns = [
  { label: '日期', prop: 'Date' },
  { label: '注册', prop: 'RegNums' },
  { label: '创角', prop: 'RoleNums' },
  { label: '创角率', prop: 'CreateRate' },
  { label: '活跃', prop: 'ActiveNums' },
  { label: '付费人数', prop: 'TotalNum' },
  { label: '付费金额', prop: 'TotalAmount' },
  { label: '付费率', prop: 'PayRate' },
  { label: 'ARPPU', prop: 'ARPPU' },
  { label: '平均在线', prop: 'AvgOnline' },
  { label: '最高在线', prop: 'MaxOnline' },
];

onMounted(async () => {
  try {
    isLoading.value = true;
    await getInfo("centre/getOperationList").then((res) => {
      isLoading.value = false;
...
const exportInfo = async () => {
  ...
  await exportGetCSV('centre/getOperationList', {
    Limit: total.value,
    Page: 1,
    ...options
  }, columns, writer, `${t("运营数据一览")}_${form.platID}_${form.serverID}_${Date.now()}.csv`).catch(async error => {
