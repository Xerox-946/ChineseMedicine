      ...
      <el-form-item>
        ...
        <el-form-item :label="$t('种类') + '：'">
          <el-select v-model="form.type" style="width:100px;margin-left:-10px;">
            <el-option :label="$t('全部')" value="all" />
            <el-option :label="$t('有偿金')" value="MaxGold" />
            <el-option :label="$t('无偿金')" value="MinGold" />
          </el-select>
        </el-form-item>
...
const loadData = async (page) => {
  ...
  const getData = await postMicro({ Cmd: "role-coin-log/getInfo", Host: "AnalysisHost", QueryType: "Get", Page: page, ...options });
...
onMounted(async () => {
  try {
    isLoading.value = true;
    await postMicro({ Cmd: "role-coin-log/getInfo", Host: "AnalysisHost", QueryType: "Get" }).then((res) => {
      isLoading.value = false;
...
const exportInfo = async () => {
  ...
  await exportMicroCSV({
    Cmd: "role-coin-log/getInfo",
    Host: "AnalysisHost",
    QueryType: "Get",
    Limit: total.value,
    Page: 1,
    ...options
  }, columns, writer, `${t('货币明细')}_${Date.now()}.csv`).catch(async error => {
