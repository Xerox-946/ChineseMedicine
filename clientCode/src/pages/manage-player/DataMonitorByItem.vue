  ...
  <el-tabs type="border-card" class="demo-tabs" v-model="curTab" @tab-click="handleTab">
    <el-form :model="form" label-width="auto">
      <el-form-item>
        ...
        <el-form-item :label="$t('道具名') + '：'" style="margin-left:-10px;">
          <el-input v-model="form.itemName" style="width:100px;margin-left:-10px;" />
        </el-form-item>
...
const loadData = async (page) => {
  const options = {
    ...
    ItemID: settingStore.getLanguage == "zh" ? itemData.value.find(item => item.Name === form.itemName)?.ItemID || 'all'
      : itemData.value.find(item => item.JP_Name === form.itemName)?.ItemID || 'all',
    Mode: form.mode,
    BeginTime: form.time[0] ? Math.floor(form.time[0].getTime() / 1000) : '0',
    EndTime: form.time[1] ? Math.floor(form.time[1].getTime() / 1000) : '0',
  }
  const getData = await postMicro({ Cmd: "role-item-log/getInfo", Host: "AnalysisHost", QueryType: "Get", Page: page, ...options });
  tableData.value = getData.data;
  total.value = getData.total;
}

const columns = [
  { label: 'ID', prop: 'ItemLogID' },
  { label: '名称', prop: 'RoleName' },
  {
    label: '道具', prop: 'ItemID', formatter: (row, column, cellValue) => {
      const item = itemData.value.find(item => item.ItemID === row.ItemID);
      return item ? settingStore.getLanguage == 'zh' ? item.Name : item.JP_Name : '未知道具';
    }
  },
  {
...
onMounted(async () => {
  try {
    isLoading.value = true;
    await postMicro({ Cmd: "role-item-log/getInfo", Host: "AnalysisHost", QueryType: "Get" }).then((res) => {
      isLoading.value = false;
...
const exportInfo = async () => {
  ...
  await exportMicroCSV({
    Cmd: "role-item-log/getInfo",
    Host: "AnalysisHost",
    QueryType: "Get",
    Limit: total.value,
    Page: 1,
    ...options
  }, columns, writer, `${t('道具明细')}_${Date.now()}.csv`).catch(async error => {
