  ...
  <el-tabs type="border-card" class="demo-tabs" v-model="curTab" @tab-click="handleTab">
    <el-form :model="form" label-width="auto">
      <el-form-item>
        ...
        <el-form-item :label="$t('家宝名称') + '：'" style="margin-left:-10px;">
          <el-input v-model="form.equipName" style="width:100px;margin-left:-10px;" />
        </el-form-item>
...
const loadData = async (page) => {
  const options = {
    ...
    EquipID: setting.getLanguage == 'zh' ? equipData.value.find(equip => equip.Name === form.equipName)?.EquipID || 'all'
      : equipData.value.find(equip => equip.JP_Name === form.equipName)?.EquipID || 'all',
    Mode: form.mode,
    BeginTime: form.time[0] ? Math.floor(form.time[0].getTime() / 1000) : '0',
    EndTime: form.time[1] ? Math.floor(form.time[1].getTime() / 1000) : '0',
  }
  const getData = await postMicro({ Cmd: "role-equip-log/getInfo", Host: "AnalysisHost", QueryType: "Get", Page: page, ...options });
  tableData.value = getData.data;
  total.value = getData.total;
}

const columns = [
  { label: 'ID', prop: 'EquipLogID' },
  { label: '名称', prop: 'RoleName' },
  {
    label: '家宝', prop: 'EquipID', formatter: (row, column, cellValue) => {
      const equip = equipData.value.find(equip => equip.EquipID === row.EquipID);
      return equip ? setting.getLanguage == 'zh' ? equip.Name : equip.JP_Name : '未知家宝';
    }
  },
  { label: '获取/消耗途径', prop: 'Cmd' },
  {
  ...
  {
    label: '特技', prop: 'EffectFightID', formatter: (row, column, cellValue) => {
      const effect = effectData.value.find(effect => effect.EffectFightID === row.EffectFightID);
      return effect ? effect.Name : '无特技';
    }
  },
  {
...
onMounted(async () => {
  try {
    isLoading.value = true;
    await postMicro({ Cmd: "role-equip-log/getInfo", Host: "AnalysisHost", QueryType: "Get" }).then((res) => {
      isLoading.value = false;
    ...
    // 获取家宝相关数据
    equipData.value = staticStore.getSysEquips;
    effectData.value = staticStore.getSysEffectFights;
  } catch (error) {
...
const exportInfo = async () => {
  ...
  await exportMicroCSV({
    Cmd: "role-equip-log/getInfo",
    Host: "AnalysisHost",
    QueryType: "Get",
    Limit: total.value,
    Page: 1,
    ...options
  }, columns, writer, `${t('家宝明细')}_${Date.now()}.csv`).catch(async error => {
